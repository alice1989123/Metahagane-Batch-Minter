...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    let amount = wasm.Value.new(wasm.BigNum.from_str("0"));
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      range.maximum = wasm.Value.new(wasm.BigNum.from_str("0")) //@ts-ignore
...
function addAmounts(amounts, compiledAmounts) {
  return compiledAmounts.checked_add(wasm.Value.from_bytes(amounts.to_bytes()));
}
...
function isQtyFulfilled(
  ...
  if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
    let minAmount = wasm.Value.new(
      wasm.min_ada_required(
        cumulatedAmount,
        wasm.BigNum.from_str(minUTxOValue.toString())
      )
    );
    ...
    if (compare(outputAmount, minAmount) < 0) {
      amount = minAmount.checked_add(
        wasm.Value.new(wasm.BigNum.from_str(protocolParameters.minUTxO))
      );
    }
